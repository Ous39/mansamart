...
if errorlevel 1 exit /b 1
docker compose --env-file "%ENV_FILE%" -f "%COMPOSE_FILE%" stop rider-mobile >nul 2>&1
echo.
...
:failed
echo.
echo MansaMart Rider did not start. Read the error above, then try again.
pause
exit /b 1
