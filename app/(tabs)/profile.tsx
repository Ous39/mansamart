...
function DashboardBanner({ title, subtitle, gradientColors, buttonLabel, buttonIcon, onPress }: {
  title: string; subtitle: string; gradientColors: readonly [string, string];
  buttonLabel: string; buttonIcon: string; onPress: () => void;
}) {
  return (
    <LinearGradient colors={gradientColors} style={styles.dashBanner}>
      <View style={styles.dashBannerLeft}>
        <Text style={styles.dashBannerTitle}>{title}</Text>
        <Text style={styles.dashBannerSubtitle}>{subtitle}</Text>
      </View>
      <Pressable
        style={({ pressed }) => [styles.dashBannerBtn, pressed && { opacity: 0.9 }]}
        onPress={onPress}
      >
        <Ionicons name={buttonIcon as any} size={14} color={gradientColors[0]} />
        <Text style={[styles.dashBannerBtnText, { color: gradientColors[0] }]}>{buttonLabel}</Text>
      </Pressable>
    </LinearGradient>
  );
}
...
function roleLabel(role: string) {
  const map: Record<string, { label: string; color: string; bg: string }> = {
    ...
    service_provider: { label: "Service Provider", color: "#7B4FA3", bg: "#F3E8FF" },
  ...
  return (
    ...
    >
      ...
      {role === "admin" && (
        <DashboardBanner
          title="Admin Panel"
          subtitle="Manage the entire MansaMart platform"
          gradientColors={["#0F172A", "#1E293B"]}
          buttonLabel="Open Panel"
          buttonIcon="shield-outline"
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); router.push("/(admin)/" as any); }}
        />
      )}
      ...
      {role === "vendor" && (
        <DashboardBanner
          ...
          subtitle="Manage products, orders & revenue"
          gradientColors={["#1F5C53", "#2B7A6E"]}
          buttonLabel="Open Dashboard"
          buttonIcon="storefront-outline"
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); router.push("/(vendor)/" as any); }}
        />
      )}
      ...
      {role === "service_provider" && (
        <DashboardBanner
          ...
          gradientColors={["#5B2D8A", "#7B4FA3"]}
          buttonLabel="Open Dashboard"
          buttonIcon="construct-outline"
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); router.push("/(provider)/" as any); }}
        />
      )}
      ...
      {role === "vendor" && (
        <View style={styles.menuSection}>
          <Text style={styles.menuSectionTitle}>Vendor Tools</Text>
          <View style={styles.menuCard}>
            <MenuItem icon="storefront-outline" label="My Dashboard" iconBg="#EFF6FF" iconColor="#2563EB" onPress={() => router.push("/(vendor)/" as any)} />
            <MenuItem icon="cube-outline" label="My Products" iconBg="#EFF6FF" iconColor="#2563EB" onPress={() => router.push("/(vendor)/products")} />
            <MenuItem icon="receipt-outline" label="Orders" iconBg="#EFF6FF" iconColor="#2563EB" onPress={() => router.push("/(vendor)/orders")} />
            <MenuItem icon="add-circle-outline" label="Add Product" iconBg="#EFF6FF" iconColor="#2563EB" onPress={() => router.push("/(vendor)/add-product")} />
          </View>
        </View>
      )}
      ...
      {role === "service_provider" && (
        <View style={styles.menuSection}>
          <Text style={styles.menuSectionTitle}>Provider Tools</Text>
          <View style={styles.menuCard}>
            <MenuItem icon="construct-outline" label="My Dashboard" iconBg="#F3E8FF" iconColor="#7B4FA3" onPress={() => router.push("/(provider)/" as any)} />
            <MenuItem icon="briefcase-outline" label="My Services" iconBg="#F3E8FF" iconColor="#7B4FA3" onPress={() => router.push("/(provider)/services")} />
            <MenuItem icon="calendar-outline" label="Bookings" iconBg="#F3E8FF" iconColor="#7B4FA3" onPress={() => router.push("/(provider)/bookings")} />
            <MenuItem icon="add-circle-outline" label="Add Service" iconBg="#F3E8FF" iconColor="#7B4FA3" onPress={() => router.push("/(provider)/add-service")} />
          </View>
        </View>
      )}
      ...
      {role === "admin" && (
        <View style={styles.menuSection}>
          ...
          <View style={styles.menuCard}>
            <MenuItem icon="people-outline" label="Manage Users" iconBg="#FEF2F2" iconColor="#DC2626" onPress={() => router.push("/(admin)/users")} />
            <MenuItem icon="cube-outline" label="Products" iconBg="#FEF2F2" iconColor="#DC2626" onPress={() => router.push("/(admin)/products")} />
            <MenuItem icon="construct-outline" label="Services" iconBg="#FEF2F2" iconColor="#DC2626" onPress={() => router.push("/(admin)/services")} />
            <MenuItem icon="receipt-outline" label="All Orders" iconBg="#FEF2F2" iconColor="#DC2626" onPress={() => router.push("/(admin)/orders")} />
          </View>
        </View>
      )}
